...
class upfile {
  ...
  public function verif_subioPorPost_muchos( $ind ) {

    if (is_uploaded_file ( $_FILES [$this->archivo] ['tmp_name'] [$ind] )) return true;
    else {
      $this->agregarError ( "El archivo no subio por post.<br>" );
      return false;
    }
  ...
  public function verif_tamanoAdecuado_muchos( $ind ) {

    if ($_FILES [$this->archivo] ['size'] [$ind] < $this->pesoMax) return true;
    else {
      $this->agregarError ( "El tamaño del archivo no esta permitido.<br>" );
      return false;
    }
  ...
  public function verif_estaVacio_muchos( $ind ) {

    if (! empty ( $_FILES [$this->archivo] ['name'] [$ind] )) return true;
    else {
      $this->agregarError ( "No existe el archivo para subir.<br>" );
      return false;
    }
  ...
  public function verif_errorEnArchivo_muchos( $ind ) {

    if ($_FILES [$this->archivo] ['error'] [$ind] == 0) return true;
    else {
      $this->AgregarError ( 'El archivo no se ha recibido correctamente.<br>' );
      return false;
    }
  ...
  public function verif_tipoValido_muchos( $ind ) {

    if (in_array ( substr ( $_FILES [$this->archivo] ['name'] [$ind], - 3 ), $this->tiposValidos )) return true;
    else {
      $this->AgregarError ( 'El tipo del archivo subido no esta permitido.<br>' );
      return false;
    }
  ...
  public function guardarArchivo_muchos( $archivo, $nombre = "" ) {

    //guardo el nombre del campo
    $this->archivo = $archivo;
    
    $cantidad = count ( $_FILES [$this->archivo] ['name'] );
    ...
    for($i = 0; $i < $cantidad; $i ++) {
      ...
      $this->nombreArchivo = "";
      //extencion
      $extencion = substr ( $_FILES [$this->archivo] ['name'] [$i], - 4 );
      //defino el nombre del archivo
      if ($nombre != "") $s = $this->nombreDelArchivo ( $nombre, $extencion );
      else if ($this->nombreArchivo == "") $s = $this->nombreDelArchivo ( substr ( $_FILES [$this->archivo] ['name'] [$i], 0, strlen ( $_FILES [$this->archivo] ['name'] [$i] ) - 4 ), $extencion );
      
      $this->nombreArchivo .= substr ( $_FILES [$this->archivo] ['name'] [$i], - 4 );
      
      //echo $this->verif_puedoEscribir(); exit();
      

      if ($s && $this->verif_subioPorPost_muchos ( $i ) && $this->verif_tamanoAdecuado_muchos ( $i ) && $this->verif_estaVacio_muchos ( $i ) && $this->verif_puedoEscribir () && $this->verif_errorEnArchivo_muchos ( $i ) && $this->verif_tipoValido_muchos ( $i ) && $this->verif_tipoValido_muchos ( $i )) {
        if (move_uploaded_file ( $_FILES [$this->archivo] ['tmp_name'] [$i], $this->directorio . "/" . $this->nombreArchivo )) {
          $this->archivos [] = $this->directorio . "/" . $this->nombreArchivo;
        } else {
          $this->agregarError ( "No se subio el archivo, no se pudo copiar la imagen" );
          return false;
        }
  ...
  public function guardarFinal( $archivo ) {

    $this->archivo = $archivo;
    if (is_array ( $_FILES [$this->archivo] ['name'] )) {
      return $this->guardarArchivo_muchos ( $this->archivo );
    } else {
      return $this->guardarArchivo ( $this->archivo );
    }
  }
}
?>